...
from agents.cv_analyzer import make_cv_analyzer_agent
from agents.jd_analyzer import make_jd_analyzer_agent
from agents.resume_tailor_agent import tailor
from agents.resume_verifier_agent import check_reworded_bullets
from agents.resume_generation_agent import generate_resume
from schemas.cv_analysis import CVProfile
from schemas.jd_analysis import JobDescriptionAnalysis
from services.cv_diff import diff_cv_profiles
...
async def generate(cv_profile: CVProfile, master_tex_path: str, output_dir: str) -> tuple[str, str]:
    return await generate_resume(cv_profile, master_tex_path, output_dir)
...
async def generate_node(state: PipelineState) -> dict:
    ...
    try:
        ...
        tex_path, pdf_path = await generate(tailored_cv, state["cv_path"], state["output_dir"])
    except Exception as e:
